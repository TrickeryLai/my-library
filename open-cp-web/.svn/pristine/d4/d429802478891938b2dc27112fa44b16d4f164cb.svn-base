...
$(document).on('mouseover','.hoverBtn',function() {
  ...
  project.trigger=setTimeout(function() {
      ...
      project.layerEle=layer.tips( lang.getData(name) , _slef , { tips: [1, common_params['rbac'].color.gray] , time:5000});
      // project.layerEle=layer.tips(globalData['name'][lang.getLang()] + lang.getData(name) , _slef , { tips: [1, common_params['rbac'].color.gray] , time:5000});
  },200); 
});

$(document).on('mouseover','.hoverBtnAll',function() {
    var _slef=this;
    // var language=lang.getLang();
    project.trigger=setTimeout(function() {
        // var name=$(_slef).attr('name');
        var name=$(_slef).attr('btnAlltype');
        project.layerEle=layer.tips( lang.getData(name) , _slef , { tips: [1, common_params['rbac'].color.gray] , time:1000});
        // project.layerEle=layer.tips(globalData['name'][lang.getLang()] + lang.getData(name) , _slef , { tips: [1, common_params['rbac'].color.gray] , time:5000});
    },200);
});
...
$('#site-search input').on('keyup',function(){
  ...
  $('#menuList [type="C"]').each(function(){
    ...
    var url=$(this).attr('url').toLowerCase();
    //保留原始字符，作为页面菜单名展示和重定向url
    var value1=$(this).html();
    var url1=$(this).attr('url');
    if(value.indexOf(val) != -1){
      str+=('<a href="javascript:;" url="'+url1+'">'+value1+'</a>');
    }
