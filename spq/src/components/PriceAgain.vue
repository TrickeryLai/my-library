      ...
      <div style="height: 100%;overflow:auto;overflow-y:scroll;">
        ...
        <van-cell-group>
          ...
          <van-row class="detail-row">
            ...
            <van-col class="detail-row-right" span="18">{{initD.acceptor}}</van-col>
          </van-row>
          <van-row class="detail-row">
            <van-col class="detail-row-left" span="6">发布人</van-col>
            <van-col class="detail-row-right" span="18">{{initD.createBy}}</van-col>
          </van-row>
          <van-row class="detail-row">
            <van-col class="detail-row-left" span="6">票据号码</van-col>
            <van-col class="detail-row-right" span="18">{{initD.cpNo}}</van-col>
          </van-row>
          <van-row class="detail-row">
            <van-col class="detail-row-left" span="6">票面金额</van-col>
            <van-col class="detail-row-right" span="18">{{initD.cpAmount && dealPrice(initD.cpAmount.toFixed(2))}}元</van-col>
          </van-row>
          <van-row class="detail-row">
            <van-col class="detail-row-left" span="6">年化利率</van-col>
            <van-col class="detail-row-right" span="18">{{initD.approvalApr && (initD.approvalApr - 0).toFixed(4)}}%</van-col>
          </van-row>
          <van-row class="detail-row">
            <van-col class="detail-row-left" span="6">背书次数</van-col>
            <van-col class="detail-row-right" span="18">{{initD.endorseTimes}}次</van-col>
          </van-row>
          <van-row class="detail-row">
            <van-col class="detail-row-left" span="6">到期时间</van-col>
            <van-col class="detail-row-right" span="18">
              {{commonFn.formatterTime(new Date(initD.dueDate), 'yyyy年MM月dd日')}}
              <span style="text-align:right;font-size: 14px;" class="blue-font">(剩{{getLastTime(item.dueDate)}}天)</span>
            </van-col>
          </van-row>
          <van-row class="detail-row" v-if="initD.actualTime">
            <van-col class="detail-row-left" span="6">成交日期</van-col>
            <van-col class="detail-row-right" span="18">
              {{commonFn.formatterTime(new Date(initD.actualTime), 'yyyy年MM月dd日')}}
            </van-col>
          </van-row>
          <van-row class="detail-row">
  ...
  export default{
    ...
    watch: {
      ...
      showState(newValue, oldValue){
        ...
        if(newValue){
          _server.buyerToPrice({
            ordNo: this.item.ordNo
          }).then(response => {
            if(response.code == 0){
              //强制更改状态
              response.data.cpStatus = '04';
              ...
              this.imgs = this.initD.frontBillImg?[_common.mosPicUrl + this.initD.frontBillImg]: [];
              if(this.initD.cpStatus == 2){
    ...
    methods: {
      ...
      transformRate(rate){
        this.initD.creditRating = 6 - this.initD.creditRating;
      },
      setTimeoutFn(){
        //如果不是报价状态，则可用取消查询
        if(this.initD.cpStatus != '04'){
          return;
        }
      ...
      showAllPrice(){
        ...
        this.priceListBaseData = this.initD;
      },
      priceListClose(){
        //查看所有报价--关闭
      ...
      getbuyPrice(){
        ...
        let _this = this, _id = this.initD.cpOrdNo;
        this.time = 60;
      ...
      previewPic(index){
        ...
        this.imagePreview = ImagePreview({
          images: [_common.mosPicUrl + this.initD.frontBillImg],
          startPosition: index,
          onClose(){
      ...
      getLastTime(){
        //获取剩余时间
        let dueDate = this.initD.dueDate;// 到期时间
        return _common.common_fn.getLastTime(dueDate);
      },
      changeData(type, value){
        let cpAmount = this.initD.cpAmount;//票据金额
        ...
        if(type == 2){
          ...
          this.submit.yearRate =((cpAmount -this.submit.dealAmount)*36000/(calDay*cpAmount)).toFixed(4);//年利率
